...
const functionMax = require('./formula/max.js');
...
Formula.prototype.shuntingYard = function() {
    var outputStream = [];
    var operatorStack = [];
...
Formula.prototype.isLowerPrecedentElement = function(element, compare) {
...
Formula.prototype.isLowerOrEqualPrecedentElement = function(element, compare) {
    var elementScore = this.getPrescedence(element);
    var compareScore = this.getPrescedence(compare);
...
Formula.prototype.isParenthesis = function(element) {
    return element === LEFT_PARENTHESIS || element === RIGHT_PARENTHESIS;
};

Formula.prototype.isOperator = function(element) {
    return element === SUM || element === SUBTRACT || element === MULTIPLY || element === DIVIDE || element === MODULE || element === EXPONENTIAL;
};

Formula.prototype.performOperation = function(element, leftOperand, rightOperand) {
...
Formula.prototype.getFunction = function(element) {
    for (var i in this.functions) {
        var f = this.functions[i];
        if (element.toLowerCase() === f.functionName.toLowerCase())
            return f;
    }
    return null;
};

Formula.prototype.isFunction = function(element) {
    return this.getFunction(element) !== null;
};

Formula.prototype.getAssociativity = function(element) {
    switch (element) {
        case SUM:
        case SUBTRACT:
        case MULTIPLY:
...
Formula.prototype.getPrescedence = function(element) {
    switch (element) {
        case SUM:
        case SUBTRACT:
            return 0;
